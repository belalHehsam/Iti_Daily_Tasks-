...
int main()
{
        ...
        SetConsoleMode(hOut, dwMode);
          ...
          cin>>sizeOfEmp;
         employees= make_unique<Employee[]>(sizeOfEmp);
         indexes=make_unique<int[]>(sizeOfEmp);
...
void newEmp(){
    system("cls");
    int index;
    cout<<"Enter the index of Employee from 0 to "<<sizeOfEmp<<" :"<<endl;
    cin>>index;
    if(index < sizeOfEmp){
        indexes[index]=1;
        system("cls");
    ...
    if(index>=sizeOfEmp){
        cout<<"Not Valid Index"<<endl;
            ...
            if(indexes[index]==1){
                    system("cls");
                     cout << "\033[1;1H" << "emp " <<":";
                     cout << "\033[2;4H" << "Id: "<<employees[index].id;
                     cout << "\033[2;" << 8 * 4 << "HAge: "<<employees[index].age;
                     cout << "\033[3;" << 8 * 4 << "HName: "<<employees[index].name;
                     cout << "\033[3;4H" << "Salary: "<<employees[index].salary <<endl;
...
void displayAll() {
    ...
    for (int i = 0; i < sizeOfEmp; i++) {
        if (indexes[i] == 1) {
            ...
            cout << "\033[" << baseRow + 1 << ";4H" << "Id: " << employees[i].id;
            cout << "\033[" << baseRow + 1 << ";" << 8 * 4 << "HAge: " << employees[i].age;
            cout << "\033[" << baseRow + 2 << ";4H" << "Name: " << employees[i].name;
            cout << "\033[" << baseRow + 2 << ";" << 8 * 4 << "HSalary: " << employees[i].salary;
